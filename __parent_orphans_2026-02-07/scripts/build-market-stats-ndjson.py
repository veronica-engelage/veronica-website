...
def parse_args():
    p = argparse.ArgumentParser(description="Build Sanity marketStatMonthly NDJSON from Realtor ZIP CSVs.")
    p.add_argument("--core-history", required=True, help="Path to RDC_Inventory_Core_Metrics_Zip_History.csv")
...
def main():
    ...
    count = 0
    with Path(args.core_history).open("r", newline="", encoding="utf-8") as f, out_path.open(
        "w", encoding="utf-8"
    ) as g:
        reader = csv.DictReader(f)
        for row in reader:
            ...
            doc = {
                ...
                "notes": "Imported from ZIP history datasets",
            }
